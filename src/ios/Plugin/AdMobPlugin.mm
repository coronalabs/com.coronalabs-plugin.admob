...
static NSString * const Y_RATIO_KEY     = @"yRatio";
...
@property (nonatomic, assign) id<CoronaRuntime> coronaRuntime;
...
CoronaAdMobDelegate *admobDelegate;
...
AdMobPlugin::load(lua_State *L)
{
    ...
    if (! isSDKInitialized(L)) { return 0; }
     ...
     loadWithCompletionHandler:^(UMPConsentForm *form, NSError *error) {
        ...
        if (error) {
           NSDictionary *coronaEvent = @{
            ...
            [admobDelegate dispatchLuaEvent:coronaEvent];
            if(adInstance){ adInstance.isLoaded = false; }
        } else {
            CoronaAdMobAdInstance *adInstance = admobObjects[@"ump"];
