...
#define PLUGIN_SDK_VERSION [GADRequest sdkVersion]
...
// saved objects (appId, ad state, etc)
...
@interface CoronaAdMobDelegate: NSObject <GADInterstitialDelegate, GADBannerViewDelegate, GADRewardBasedVideoAdDelegate>
...
AdMobPlugin::load(lua_State *L)
{
  ...
  // set child safe flag
  [request tagForChildDirectedTreatment:childSafe];
  ...
  if ((localTestModeIsSet && localTestMode) || ((!localTestModeIsSet) && globalTestMode)) {
    ...
    request.testDevices = @[kGADSimulatorID, admobDeviceId];
  ...
  if (UTF8IsEqual(adType, TYPE_INTERSTITIAL)) {
    ...
    GADRewardBasedVideoAd *rewardedAd = [GADRewardBasedVideoAd sharedInstance];
    
    // create ad instance object (stores additional info about the ad not available in GADRewardBasedVideoAd)
    ...
    [rewardedAd setDelegate:admobDelegate]; // need to set delegate here since the rewardedAd is a sharedInstance
    [rewardedAd loadRequest:request withAdUnitID:@(adUnitId)];
  }
...
AdMobPlugin::show(lua_State *L)
{
  ...
  if (UTF8IsEqual(adType, TYPE_INTERSTITIAL)) {
    ...
    GADRewardBasedVideoAd *rewardedAd = (GADRewardBasedVideoAd *)adInstance.adInstance;
    if (! rewardedAd.isReady) {
      logMsg(L, WARNING_MSG, MsgFormat(@"Rewarded Video not loaded for adUnitId '%@'", adUnitId));
      return 0;
    }
    
    [rewardedAd presentFromRootViewController:library.coronaViewController];
  }
  else if (UTF8IsEqual(adType, TYPE_BANNER)) {
...
- (NSString *)getJSONStringForRewardedAd:(GADRewardBasedVideoAd *)ad
{
  return [self getJSONStringForRewardedAd:ad reward:nil];
}

- (NSString *)getJSONStringForRewardedAd:(GADRewardBasedVideoAd *)ad reward:(GADAdReward *)reward
{
  return [self getJSONStringForRewardedAd:ad reward:reward error:nil];
}

- (NSString *)getJSONStringForRewardedAd:(GADRewardBasedVideoAd *)ad reward:(GADAdReward *)reward error:(NSError *)error
{
  NSMutableDictionary *dataDictionary = [NSMutableDictionary new];
...
- (void)rewardBasedVideoAd:(GADRewardBasedVideoAd *)rewardBasedVideoAd didRewardUserWithReward:(GADAdReward *)reward
{
  ...
  NSDictionary *coronaEvent = @{
    ...
    CORONA_EVENT_DATA_KEY : [self getJSONStringForRewardedAd:rewardBasedVideoAd reward:reward]
  };
  [self dispatchLuaEvent:coronaEvent];
}
...
- (void)rewardBasedVideoAd:(GADRewardBasedVideoAd *)rewardBasedVideoAd didFailToLoadWithError:(NSError *)error
{
  ...
  NSDictionary *coronaEvent = @{
    ...
    @(CoronaEventIsErrorKey()) : @(true),
    @(CoronaEventResponseKey()) : RESPONSE_LOAD_FAILED,
    CORONA_EVENT_DATA_KEY : [self getJSONStringForRewardedAd:rewardBasedVideoAd reward:nil error:error]
  };
  [self dispatchLuaEvent:coronaEvent];
...
- (void)rewardBasedVideoAdDidReceiveAd:(GADRewardBasedVideoAd *)rewardBasedVideoAd
{
  ...
  NSDictionary *coronaEvent = @{
    @(CoronaEventPhaseKey()) : PHASE_LOADED,
    @(CoronaEventTypeKey()) : @(TYPE_REWARDEDVIDEO),
    CORONA_EVENT_DATA_KEY : [self getJSONStringForRewardedAd:rewardBasedVideoAd]
  };
  [self dispatchLuaEvent:coronaEvent];
...
- (void)invalidateInfo
{
  if (self.adInstance != nil) {
    ...
    if (UTF8IsEqual([self.adType UTF8String], TYPE_BANNER)) {
      ...
      GADRewardBasedVideoAd *rewardedAd = (GADRewardBasedVideoAd *)self.adInstance;
      [rewardedAd setDelegate:nil];
    }
